...
def sneaker_not_found_handler(request: Request, exc: SneakerNotFound):
    return JSONResponse(status_code=404, content={"detail": "Sneaker not found"})


def sneaker_already_exists_handler(request: Request, exc: SneakerAlreadyExists):
    return JSONResponse(status_code=409, content={"detail": "Sneaker already exists"})


def sneaker_is_inactive_handler(request: Request, exc: SneakerIsInactive):
    return JSONResponse(status_code=409, content={"detail": "Sneaker is inactive"})


def one_time_subscription_is_active_handler(request: Request, exc: OneTimeSubscriptionIsActive):
    return JSONResponse(status_code=409, content={"detail": "One time subscription is active"})


def one_time_subscription_already_exists_handler(request: Request, exc: OneTimeSubscriptionAlreadyExists):
    return JSONResponse(status_code=409, content={"detail": "One time subscription already exists"})


def one_time_subscription_not_found_handler(request: Request, exc: OneTimeSubscriptionNotFound):
    return JSONResponse(status_code=404, content={"detail": "One time subscription not found"})


def permanent_subscription_is_active_handler(request: Request, exc: PermanentSubscriptionIsActive):
    return JSONResponse(status_code=409, content={"detail": "Permanent subscription is active"})


def permanent_subscription_already_exists_handler(request: Request, exc: PermanentSubscriptionAlreadyExists):
    return JSONResponse(status_code=409, content={"detail": "Permanent subscription already exists"})


def permanent_subscription_not_found_handler(request: Request, exc: PermanentSubscriptionNotFound):
