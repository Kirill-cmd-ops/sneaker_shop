from sqlalchemy import select
from sqlalchemy.exc import IntegrityError

from microservices.auth_service.auth_service.auth.domain.exceptions import UserNotFound, RoleNotFound, \
    UserRoleAssociationAlreadyExists
from microservices.auth_service.auth_service.auth.models import db_helper, User, Role, UserRoleAssociation


async def add_role_db(user_id: int, role_name: str) -> None:
    try:
        async with db_helper.session_context() as session:
            user_object = await session.get(User, user_id)
            if not user_object:
                raise UserNotFound()

            role_id = await session.scalar(select(Role.id).where(Role.name == role_name))
            if not role_id:
                raise RoleNotFound()

            user_role_association = UserRoleAssociation(user_id=user_id, role_id=role_id)
            session.add(user_role_association)
            await session.commit()
    except IntegrityError:
        raise UserRoleAssociationAlreadyExists()
