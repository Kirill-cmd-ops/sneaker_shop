...
from stock_notification_service.stock_notification.services.subscription.one_time.reactivate import (
    reactivate_user_one_time_subscription_service,
    reactivate_one_time_subscription_service,
)
...
async def create_user_one_time_subscription(
    ...
    async with session.begin():
        try:
            ...
            await is_size_available_for_sneaker(
                session=session,
                sneaker_id=subscription_create.sneaker_id,
            ...
            update_subscription = await reactivate_one_time_subscription_service(
                session=session,
                user_id=user_id,
                sneaker_id=subscription_create.sneaker_id,
                size_id=subscription_create.size_id,
...
async def reactivate_all_one_time_subscriptions_for_user(
    ...
    async with session.begin():
        ...
        try:
            return reactivate_user_one_time_subscription_service(
                subscription_id=subscription_id,
                user_id=user_id,
                session=session,
            )
