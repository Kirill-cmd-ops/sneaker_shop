...
export default function Home() {
  const [newsneakers, setNewSneakers] = useState(null);
  const [nikesneakers, setNikeSneakers] = useState(null);
  const [pumasneakers, setPumaSneakers] = useState(null);
  const [vanssneakers, setVansSneakers] = useState(null);
  const [adidassneakers, setAdidasSneakers] = useState(null);
  ...
  const fetchSneakers = async (url, setter) => {
    try {
      ...
      setter(data || { total_count: 0, items: [] });
    } catch (error) {
      console.error("Ошибка загрузки данных:", error);
      setter({ total_count: 0, items: [] });
    }
  };
